...
export const updateProfile = (userData) => async (dispatch) => {
	try {
		...
		const { data } = await axios.put("http://localhost:8000/api/me/update", userData, config);
...
export const updatePassword = (oldPassword, password) => async (dispatch) => {
	try {
		...
		const { data } = await axios.put(
			"http://localhost:8000/api/password/update",
...
export const forgotPassword = (email) => async (dispatch) => {
	try {
		...
		const { data } = await axios.post("http://localhost:8000/api/password/forgot", email, config);
